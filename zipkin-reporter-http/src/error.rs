...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.inner {
            ErrorInner::Hyper(ref e) => fmt::Display::fmt(e, f),
            ErrorInner::Http(c) => write![f, "zipkin server replied with status code {}", c],
        }
    }
}
...
impl error::Error for Error {
    ...
    fn description(&self) -> &str {
        match self.inner {
            ErrorInner::Hyper(ref e) => error::Error::description(e),
            ErrorInner::Http(_) => "zipkin server replied with bad status code",
        }
    }
}
...
mod test {
    ...
    fn it_works_for_hyper_errors() {
        ...
        drop(body);
        let err = Error {
            inner: ErrorInner::Hyper(sender.poll_ready().unwrap_err()),
        };
    ...
    fn it_works_for_status_code() {
        let err = Error {
            inner: ErrorInner::Http(http::StatusCode::INTERNAL_SERVER_ERROR),
        };
        assert![err.is_http_error()];
        assert_eq![
            err.to_string(),
            "zipkin server replied with status code 500 Internal Server Error"
        ];
