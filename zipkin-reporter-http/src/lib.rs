...
pub(crate) fn resolve_spans_path(uri: http::Uri) -> http::Uri {
    ...
    parts.path_and_query = match parts.path_and_query {
        ...
        None => Some(http::uri::PathAndQuery::from_static("/api/v2/spans")),
    };
    http::Uri::from_parts(parts)
        .expect("Invalid Uri supplied to zipkin_reporter_http::Builder::new")
}
...
impl<C: hyper::client::connect::Connect> Builder<C> {
    ...
    pub fn client<D: hyper::client::connect::Connect>(
        self,
        client: hyper::Client<D, hyper::Body>,
    ) -> Builder<D> {
        Builder {
            client,
            uri: self.uri,
    ...
    C::Future: 'static,
{
        ...
        F: Send + Fn(Error) + 'static,
    {
        ...
        let handle = thread::Builder::new()
            ...
            .spawn(move || {
                hyper::rt::run(worker.map_err(error_handler).for_each(|_| Ok(())));
            })
            .unwrap();
        (handle, reporter)
    }
...
mod test {
    ...
    fn test_server<F>(port: u16, responder: F) -> mpsc::Receiver<hyper::Request<Vec<u8>>>
    where
        F: 'static + Send + Clone + Fn(&hyper::Request<Vec<u8>>) -> hyper::Response<hyper::Body>,
    {
        let (tx, rx) = mpsc::sync_channel(10);
        let server = hyper::Server::bind(&([127u8, 0, 0, 1], port).into()).serve(move || {
            let tx = tx.clone();
        ...
        thread::spawn(move || hyper::rt::run(server.map_err(|e| eprint!["{:?}", e])));
        return rx;
    }
    ...
    fn it_should_report() {
        ...
        assert_eq![
            ...
            Some(&hyper::header::HeaderValue::from(body.len()))
        ];
